  ...
  <div>
    ...
    <div class="el-dropdown-link">
      ...
      <el-button id="search" type="primary" icon="el-icon-search"
        style="margin-top: 5px; margin-right: -200px; position: inherit" @click="searchCourse">
        查询
      </el-button>
    </div>
    <!-- 教师课表整体 -->
            ...
            <div v-for="number in numbers" :key="number" class="class_row">
              <div v-for="course in courseData" :key="course.index">
                <div v-if="
                  ...
                  hasCourseFlag
                  " class="course_info" @click="showCourseDetails(course)">
                  <h3 style="font-size: 18px; margin-top: 7px">
                    {{ course.courseName }}
                  </h3>
                  <div class="smallInfo">
                    <p>班级编号：{{ course.classID }}</p>
...
export default {
  ...
  methods: {
    ...
    searchCourse() {
      ...
      this.$axios
        .post("http://192.168.1.119:8080/weekTeacherCourse", form)
        .then((resp) => {
